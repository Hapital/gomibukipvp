...
$execute as @a if score @s lib.gun.uid = @e[type=armor_stand,tag=Lib.Gun.Bullet,sort=nearest,limit=1] lib.gun.bulletOwner run damage @e[tag=Lib.Gun.Hit,sort=nearest,limit=1] $(damage_head) lib:gun by @s
